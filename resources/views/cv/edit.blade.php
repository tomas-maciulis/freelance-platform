...
@section('content')
    ...
    <form method="post" action="{{ route('cv.update', $cv->id) }}" class="bg-white w-full px-8 pt-6 pb-8 mb-4 relative">
        ...
        <div class="mb-4">
            ...
            <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="introduction" type="text" name="introduction" rows="15" maxlength="10000">{{ old('introduction') ?? $cv->introduction }}</textarea>
            @error('introduction')
            <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="qualification" type="text" name="qualification" rows="15" maxlength="10000">{{ old('qualification') ?? $cv->qualification }}</textarea>
            @error('qualification')
            <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            @enderror
        </div>
